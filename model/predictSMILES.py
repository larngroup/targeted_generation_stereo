...
class BasePredictSMILES(object):
    ...
    def __init__(self, model_unbiased,model_biased,training, threshold,config,sampling):
...
class PredictSMILES(BasePredictSMILES):
    ...
    """
    def __init__(self, model_unbiased,model_biased,training,threshold,config,sampling):
        super(PredictSMILES, self).__init__(model_unbiased,model_biased,training,threshold, config,sampling)
        self.model_unbiased = model_unbiased
        self.model_biased = model_biased
    ...
    def sample_with_temp(self, preds): 
        ...
        if self.sampling == True:
            streched = np.log(preds) /self.config.sample_temp #0.67
        else:
            streched = np.log(preds) /self.config.train_temp #0.92
